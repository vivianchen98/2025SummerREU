            ...
            if self.catchExceptions:
                try:
                    timed_func = TimeoutFunction(agent.observationFunction,
                                                 int(self.rules.getMoveTimeout(self.agentIndex)))
                    try:
                        start_time = time.time()
        ...
        if self.catchExceptions:
            try:
                timed_func = TimeoutFunction(agent.getAction,
                                             int(self.rules.getMoveTimeout(self.agentIndex)) - int(move_time))
                try:
                    start_time = time.time()
                    if skip_action:
                ...
                if self.totalAgentTimes[self.agentIndex] > self.rules.getMaxTotalTime(self.agentIndex):
                    print(
                        "Agent %d ran out of time! (time: %1.2f)" % (self.agentIndex, self.totalAgentTimes[self.agentIndex]),
                        file=sys.stderr)
                    self.agentTimeout = True
                    self._agentCrash(self.agentIndex, quiet=True)
                    self.unmute()
